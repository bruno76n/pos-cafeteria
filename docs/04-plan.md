...
- [ ] 0.3 Proyecto Vite + React + TypeScript armado a mano (sin asistentes): `package.json`, `index.html` (lang `es-MX`), `vite.config.ts`, `tsconfig*.json` (strict, que incluya `src/`, `servidor/`, `api/` y `scripts/`), `src/main.tsx`, `src/App.tsx`, alias `@/` → `src/`. → `npm run dev` sirve una página.
- [ ] 0.4 Tailwind CSS v4 (`@tailwindcss/vite`), tokens de `docs/03-interfaz.md` en `src/estilos/global.css` y fuente autoalojada. → Página de prueba con los colores y la fuente.
- [ ] 0.5 ESLint (typescript-eslint, react-hooks) + Prettier. Scripts `lint`, `typecheck`, `format`. → Los tres corren sin errores.
- [ ] 0.6 Vitest (entornos `jsdom` para la app y `node` para el servidor) con una prueba trivial de cada uno. Script `test`. → `npm test` pasa.
...
- [ ] 0.8 `api/[[...ruta]].ts` con el adaptador de Hono para Vercel y `vercel.json` (`docs/02-arquitectura.md` §12). → Compila (no se despliega).
