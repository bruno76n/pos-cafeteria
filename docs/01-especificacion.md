...
| Grupo de modificadores | Conjunto de opciones (Tamaño, Leche, Jarabes…) que se asigna a productos. |
...
| Crear y editar productos, categorías y modificadores | `crearProductos` | ✓ | ✓ | – |
...
- **Tocar un producto sin modificadores** agrega una línea (cantidad 1). Si ya existe una línea idéntica (mismo producto, sin modificadores, sin nota), suma 1 a esa línea.
- **Tocar un producto con modificadores** abre la hoja de personalización con las opciones por defecto ya elegidas: validación de obligatorio / mínimo / máximo, campo de nota, cantidad y botón "Agregar $85.00" con el precio calculado. Líneas idénticas (mismo producto, mismas opciones, sin nota) se fusionan.
- **Carrito:** cada línea muestra cantidad, nombre, resumen de opciones ("Mediano, Almendra, 1 shot extra"), nota e importe. Acciones por línea: + / −, eliminar, editar (reabre la hoja), nota.
...
Contenido: logo, nombre de la cafetería, dirección, teléfono y RFC (si aplica y está activado), folio, fecha y hora, dispositivo, cajero, "Para:" si hay, productos con cantidad, opciones y nota, precio unitario cuando la cantidad es mayor a 1, importes, subtotal, descuento, total, desglose de IVA (si está activado), pagos (recibido y cambio en efectivo; referencia en tarjeta/transferencia) y mensaje final.
...
Campos: nombre (obligatorio), imagen (opcional), categoría (obligatoria), precio (≥ 0), descripción, disponible / no disponible, orden en el menú y grupos de modificadores asignados (en orden).

Ejemplo: **Latte**, $65.00, "Espresso con leche vaporizada", con los grupos Tamaño, Leche, Extra shot, Jarabes y Toppings.
...
- El precio solo se edita con `modificarPrecios`; sin ese permiso el campo es de solo lectura. El resto requiere `crearProductos`.
...
### 5.6 Menú: categorías y modificadores
...
**Grupos de modificadores:** nombre ("Tamaño"), tipo (una opción / varias opciones), obligatorio sí/no, mínimo y máximo (para varias opciones), orden y opciones. Cada opción: nombre, precio extra (≥ 0), por defecto sí/no y disponible sí/no. Un grupo se reutiliza en muchos productos.

Ejemplo:

| Grupo | Tipo | Obligatorio | Opciones |
|---|---|---|---|
| Tamaño | una | sí | Chico +$0 (por defecto), Mediano +$10, Grande +$20 |
...
- `precioUnitario = precioBase + Σ precioExtra de las opciones elegidas`
...
|---|---|---|
| A | Latte $65 + Mediano $10 + Almendra $10, cantidad 2; Brownie $45 | precio unitario $85; subtotal $215.00 |
...
| J | Latte $65 + Grande $20 + Avena $10 + 1 shot $15 + Vainilla $10 + Caramelo $10 | $130; una tercera opción de Jarabes (máx. 2) no se permite |
